...
QString BrowserService::storeKey(const QString& key)
{
    ...
    do {
        ...
        if (contains) {
            dialogResult = MessageBox::warning(nullptr, tr("KeePassXC: Overwrite existing key?"),
                                 ...
                                 const QString& submitUrl)
{
    ...
    if (username.compare(login, Qt::CaseSensitive) != 0 || entry->password().compare(password, Qt::CaseSensitive) != 0) {
        ...
        if (!browserSettings()->alwaysAllowUpdate()) {
            dialogResult = MessageBox::question(nullptr, tr("KeePassXC: Update Entry"),
...
void BrowserService::convertAttributesToCustomData(QSharedPointer<Database> currentDb)
{
    ...
    if (counter > 0) {
        MessageBox::information(nullptr, tr("KeePassXC: Converted KeePassHTTP attributes"),
        ...
        MessageBox::information(nullptr, tr("KeePassXC: No entry with KeePassHTTP attributes found!"),
